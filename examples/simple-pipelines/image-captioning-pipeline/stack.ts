...
/**
 * Example stack for captioning images using the BLIP2 model.
 * The pipeline looks as follows:
 *
 * ┌──────────────┐   ┌────────────────────────┐   ┌───────────────┐
 * │  S3 Trigger  ├──►│  BLIP2 Image Processor ├──►| Output Bucket |
 * └──────────────┘   └────────────────────────┘   └───────────────┘
 *
 * @see https://github.com/huggingface/blog/blob/main/blip-2.md
 */
export class ImageCaptioning extends cdk.Stack {
  ...
  constructor(scope: Construct, id: string, env: cdk.StackProps) {
    ...
    trigger
      // Caption images using the BLIP2 model.
      .pipe(
        new Blip2ImageProcessor.Builder()
          .withScope(this)
...
// Deploy the stack.
new ImageCaptioning(app, 'ImageCaptioning', {
  env: {
    account,
    region
  }

 ...
 * to summarize text using the HuggingFace transformers
 * library using abstractive summarization. The pipeline
 * looks as follows:
 *
 *
 * ┌──────────┐   ┌───────────────────────┐   ┌───────────┐
 * │ S3 Input ├──►│ Summarizer Middleware │──►│ S3 Output │
 * └──────────┘   └───────────────────────┘   └───────────┘
 *
 *
 ...
 */
export class AbstractiveSummarizationStack extends cdk.Stack {
  ...
  constructor(scope: Construct, id: string, env: cdk.StackProps) {
    super(scope, id, {
      description: 'A pipeline summarizing text documents using Transformers models.',
      ...env
    });
    ...
    trigger
      // Summarize text documents using the HuggingFace
      // transformers library.
      .pipe(
        new TransformersTextSummarizer.Builder()
          .withScope(this)
          .withIdentifier('TextSummarizer')
          .withCacheStorage(cache)
          .withSource(trigger)
          .withVpc(vpc)
...
// Deploy the stack.
new AbstractiveSummarizationStack(app, 'AbstractiveSummarizationStack', {
  env: {
    account,
    region
  }

...
/**
 * An example stack showcasing how to apply topic modeling
 * on text documents using the KeyBERT model.
 *
 * The pipeline looks as follows:
 *
 * ┌──────────────┐    ┌────────────────────────┐    ┌─────────────┐
 * │   S3 Input   ├───►│  KeybertTextProcessor  ├───►│  S3 Output  │
 * └──────────────┘    └────────────────────────┘    └─────────────┘
 *
 */
export class TopicModelingStack extends cdk.Stack {
  ...
  constructor(scope: Construct, id: string, env: cdk.StackProps) {
    ...
    trigger
      // Extract the main topics from text documents.
      .pipe(
        new KeybertTextProcessor.Builder()
          .withScope(this)
          .withIdentifier('KeybertTextProcessor')
          .withCacheStorage(cache)
          .withVpc(vpc)
          .withSource(trigger)
          // Optionally specify the top n topics to retrieve.
          .withTopN(10)
          .build()
      )
...
// Deploy the stack.
new TopicModelingStack(app, 'TopicModelingStack', {
  env: {
    account,
    region
  }

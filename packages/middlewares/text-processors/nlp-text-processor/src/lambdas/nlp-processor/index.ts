...
const ops = JSON.parse(process.env.NLP_OPS ?? '[]');
...
class Lambda implements LambdaInterface {
  ...
  async processEvent(event: CloudEvent): Promise<CloudEvent> {
    ...
    // Execute analysis on the document.
    const metadata = await nlp(document, ops);

    // Merge the gathered metadata with the existing metadata.

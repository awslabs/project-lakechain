 ...
 */
const TEXT_MODEL = JSON.parse(process.env.TEXT_MODEL as string) as Llama2TextModel;
...
class Lambda implements LambdaInterface {
  ...
  private async getPrompt(event: CloudEvent) {
    return (`[INST]${await event.resolve(PROMPT)}[/INST]\n\n${await event.resolve(DOCUMENT)}`);
  }

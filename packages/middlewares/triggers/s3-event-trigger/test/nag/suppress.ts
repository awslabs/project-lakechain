...
import { acknowledge, acknowledgeByPath } from './acknowledge';
...
export const suppressNagS3EventTrigger = (stack: Stack, bucket: Bucket) => {
  ...
  const bucketNotificationHandlerPolicies = stack.node
      .findAll()
      .filter((construct) => construct.node.id === 'HandlerPolicy');

  if (bucketNotificationHandlerPolicies.length > 0) {
    acknowledge(
        bucketNotificationHandlerPolicies,
        [

...
> 💁 Note that you will need to specify a data source that the BLIP2 processor will use as an input, such as the [S3 trigger](/triggers/s3-event-trigger).
...
<details>
  ...
  > ℹ️ Below is an example of a [CloudEvent](/general/events) emitted by the BLIP2 processor.

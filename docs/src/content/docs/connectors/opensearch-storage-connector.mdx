...
import icon from '../../../assets/icon-opensearch.svg';
...
<span class="language-icon" style="margin-right: 10px">
  <a target="_blank" href="https://opensearch.org/">
...
The OpenSearch storage connector enables developers to index [CloudEvents](/project-lakechain/general/events) in an [Amazon OpenSearch](https://opensearch.org/) domain at scale within their pipelines. This connector uses [AWS Firehose](https://aws.amazon.com/firehose/) to buffer events and store them in batch to OpenSearch using a serverless architecture.

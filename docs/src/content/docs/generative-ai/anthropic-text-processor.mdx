...
import icon from '../../../assets/icon-anthropic.svg';
...
In addition to handling single documents, the Anthropic text processor also supports [composite events](/project-lakechain/general/events#-composite-events) as an input. This means that it can take multiple text and image documents and compile them into a single input to the model.

This can come in handy in map-reduce pipelines where you use the [Reducer](/project-lakechain/flow-control/reducer) to combine multiple documents into a single input having a similar semantic, for example, multiple pages of a PDF document that you would like the model to summarize as a whole, while keeping the context between the pages.
...
- [Claude Summarization Pipeline](https://github.com/awslabs/project-lakechain/tree/main/examples/simple-pipelines/summarization-pipelines/claude-summarization-pipeline) - Builds a pipeline for text summarization using Amazon Bedrock and Anthropic Claude.
